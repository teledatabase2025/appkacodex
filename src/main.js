...
const auditLog = ['ORION_BOOT: systém aktivní', 'AUTH: detektiv ověřen', 'CASE_LOAD: 2254578/2026'];
...
function render() {
  document.querySelector('#app').innerHTML = `
    <aside class="sidebar">
      ...
      <section class="terminal-log"><header>auditní log</header>${auditLog.map((entry) => `<code>${escapeHtml(entry)}</code>`).join('')}</section>
    </aside>
