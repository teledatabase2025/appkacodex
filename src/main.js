...
const slides = [
  {
    ...
    details: [
      ['Režim', 'Detektiv'], ['Případ', '2254578/2026'], ['Interní označení', 'ŠEPOTY STROMŮ'], ['STAV PŘÍPADU', 'ČEKÁ NA ZAHÁJENÍ'], ['Systém', 'Aktivní'],
    ],
    videoUrl: VIDEO_URL,
  },
...
function renderQuestion(slide) {
  ...
  if (slide.type === 'single' || slide.type === 'ack') {
    return `<div class="option-grid">${slide.options.map((option) => `<button class="option ${answer === option ? 'selected' : ''}" data-action="choose" data-value="${escapeHtml(option)}"><span class="option-marker"></span>${escapeHtml(option)}</button>`).join('')}</div><button class="primary" data-action="submit">Potvrdit odpověď</button>`;
  }
  if (slide.type === 'multi') {
...
function renderCaseStatus() {
  const label = state.index === 0 ? 'STAV PŘÍPADU: ČEKÁ NA ZAHÁJENÍ' : 'STAV PŘÍPADU: PROBÍHÁ VYŠETŘOVÁNÍ';
  return `<span class="case-status">${label}</span>`;
}

function render() {
  const root = document.querySelector('#app');
  const slide = currentSlide();
  root.className = `app-shell view-${slide.kind}${slide.type ? ` question-${slide.type}` : ''}`;
  root.innerHTML = `
    <aside class="sidebar">
      ...
      <section class="locked-panel"><span>Navigace</span><strong>Sekvenční režim</strong><p>Slidy se odemykají pouze postupně podle průběhu vyšetřování.</p></section>
    ...
    <section class="workspace">
      <header class="topbar"><div><span class="eyebrow">● SYSTÉM AKTIVNÍ</span><h3>Interní systém O.R.I.O.N.</h3></div><div class="top-actions">${renderCaseStatus()}<button data-action="sound">Test spojení</button></div></header>
      ${renderSlide(slide)}
    </section>`;
}
